    ...
    for (int j=0; j<(found.size()>0 ? found[0].size() : 0); j++) {      // index into search_files
        ...
        for (int i = 0; i < neighbors_depth1.size(); i++) {
            ...
            if (!done && found[i][j]==1) {
                ...
                cout << "Found " << search_files[j] << " at " << neighbors_depth1[i]->pvt_id << " with MD5 0 at depth 1" << endl;
        ...
        if (!done) {
            ...
            for (int i = neighbors_depth1.size(); i < neighbors_depth1.size()+neighbors_depth2.size(); i++) {
                ...
                if (!done && found[i][j]==1) {
                    ...
                    cout << "Found " << search_files[j] << " at " << neighbors_depth2[i-neighbors_depth1.size()]->pvt_id << " with MD5 0 at depth 2" << endl;
